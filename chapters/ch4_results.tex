...
Grammar Correction     & Typhoon2-8B (Ollama)  & 800--1{,}500~ms    & \texttt{temp=0.1}, max 256 tokens \\ \hline
RAG Retrieval          & Milvus + bge-m3       & 50--200~ms          & IVF\_FLAT, top-k=3 \\ \hline
Memory Retrieval       & Milvus + MySQL        & 30--80~ms           & Filtered by \texttt{student\_id} \\ \hline
LLM Inference          & Typhoon2-70B (Ollama) & 4{,}000--12{,}000~ms & \texttt{temp=0.7}, max 512 tokens \\ \hline
...
\textbf{Time to First Reaction} & ---          & \textbf{6{,}000--15{,}000~ms} & End-to-end per turn \\ \hline
\end{tabular}
}%
}
\end{table}
...
The latency benchmark results recorded a median end-to-end total latency of 6,634~ms and a 95th-percentile latency of 15,775~ms. Most RAG routes perform within the 6,000--8,000~ms range, with \textbf{uni\_info} and \textbf{local\_info} being the fastest routes due to their smaller and simpler collections. The \textbf{student\_manual} route is a notable outlier, recording the highest median latency of 15,775~ms and a p95 of 21,502~ms, likely attributable to denser semantic matching over longer procedural content. Pipeline stage analysis indicates that LLM inference accounts for the majority of total latency, with a median of 6,609~ms and a p95 of 15,678~ms, identifying model generation rather than retrieval as the primary bottleneck. Full per-route and per-stage breakdown is presented in the table above.
\begin{table}[h]
\captionsetup{justification=raggedright,singlelinecheck=false,margin=0pt,indention=0pt}
...
\texttt{time\_table}   & 3  & 3,245 & 2,546 & 2,837 \\ \hline
\texttt{local\_info}   & 3  & 6,022 & 6,621 & 6,765 \\ \hline
